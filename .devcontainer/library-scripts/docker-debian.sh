...
echo "Function to run apt-get if needed"
...
echo  "Install Docker / Moby CLI if not already installed"
...
echo "If init file already exists, exit"
# If init file already exists, exit
if [ -f "/usr/local/share/docker-init.sh" ]; then
    exit 0
fi
echo "By default, make the source and target sockets the same"
# By default, make the source and target sockets the same
if [ "${SOURCE_SOCKET}" != "${TARGET_SOCKET}" ]; then
    touch "${SOURCE_SOCKET}"
    ln -s "${SOURCE_SOCKET}" "${TARGET_SOCKET}"
fi
echo "Add a stub if not adding non-root user access, user is root"
...
echo "If enabling non-root access and specified user is found, setup socat and add script"
